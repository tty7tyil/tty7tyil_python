...
class MD_Data:
    def __init__(self, md_file_content: str):
        ...
        self.toc: tree.Tree[TOC_Header_Data] = MD_Data.extract_toc(md_file_content)
    ...
    @staticmethod
    def extract_toc(md_file_content: str) -> tree.Tree[TOC_Header_Data]:
        ...
        for e in raw_toc_list:
            toc_header_data_list.append(TOC_Header_Data(len(e[0]), e[1], serial_dict[e[1]]))
            serial_dict[e[1]] += 1

        return MD_Data.__fill_toc_tree(
            tree.Tree(TOC_Header_Data(0, 'Table of Contents', 0)),
            toc_header_data_list,
        )[0]

    @staticmethod
    def __fill_toc_tree(
        toc_tree: tree.Tree[TOC_Header_Data],
        toc_header_data_list: List[TOC_Header_Data],
        /,
        start_index: int = 0,
    ) -> Tuple[tree.Tree[TOC_Header_Data], int]:
